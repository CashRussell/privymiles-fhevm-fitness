  ...
  useEffect(() => {
    ...
    if (_isRunning === true) {
      ...
      createFhevmInstance({
        ...
        .catch((e) => {
          // Don't log or set error state if operation was aborted (e.g., during route navigation)
          if (thisSignal.aborted) {
            // Silently ignore abort errors - they're expected during component unmount or route changes
            return;
          }
          
          console.error(`[useFhevm] Error creating FHEVM instance:`, e);
          console.error(`[useFhevm] Error name: ${e.name}, message: ${e.message}`);
          if (e.stack) {
