...
export class RelayerSDKLoader {
  ...
  public load(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      // Check if script is already loaded (either from CDN or local)
      ...
      // Try loading from CDN first, fallback to local file
      const tryLoad = (url: string, isFallback: boolean = false) => {
        ...
        script.onload = () => {
          if (!isFhevmWindowType(window, this._trace)) {
            ...
            if (isFallback) {
              reject(
                new Error(
                  `RelayerSDKLoader: Failed to load Relayer SDK from both CDN and local file.`
                )
              );
            } else {
              // Try local file as fallback
              console.log("[RelayerSDKLoader] CDN failed, trying local file...");
              tryLoad(SDK_LOCAL_URL, true);
            }
            return;
          }
          console.log(`[RelayerSDKLoader] Successfully loaded from ${isFallback ? 'local file' : 'CDN'}`);
          resolve();
        };

        script.onerror = () => {
          console.log(`[RelayerSDKLoader] script onerror from ${url}...`);
          if (isFallback) {
            reject(
              new Error(
                `RelayerSDKLoader: Failed to load Relayer SDK from both CDN (${SDK_CDN_URL}) and local file (${SDK_LOCAL_URL})`
              )
            );
          } else {
            // Try local file as fallback
        ...
        console.log(`[RelayerSDKLoader] add script to DOM from ${url}...`);
